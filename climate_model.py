...
class ClimateModel:
    ...
    def __init__(self, debug=False):
        self.debug = debug
        self.forcings = pd.read_csv(Path('data/AR6_ERF_1750-2024.csv'))
        ...
        self.hadcrut5 = self.hadcrut5[self.hadcrut5.Year <= 2024]
    ...
    def run_model(self, total_forcing=None, **control_values):
        ...
        dTm_match_obs = dT[(dT.year >= 1850) & (dT.year <= 2024)].dTm - self.anomaly_baseline.dTm
        self.rmse = np.sqrt(
        ...
        if ax1 is None:
            ...
            if show_obs:
                if self.control_values:
                    # looks in the wrong order so plot titles wrong
                    #lam, d_d, d_m, K, aer_cld_scaling = [
                    lam, d_m, d_d, K, aer_cld_scaling = [                    
